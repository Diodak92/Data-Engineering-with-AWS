...
FROM (
    ...
    CAST(from_unixtime(registrationDate / 1000) AS DATE) AS registrationDate, 
    CAST(from_unixtime(lastUpdateDate / 1000) AS DATE) AS lastUpdateDate, 
    CAST(from_unixtime(shareWithResearchAsOfDate / 1000) AS DATE) AS shareWithResearchAsOfDate,
    CAST(from_unixtime(shareWithPublicAsOfDate / 1000) AS DATE) AS shareWithPublicAsOfDate,
    CAST(from_unixtime(shareWithFriendsAsOfDate / 1000) AS DATE) AS shareWithFriendsAsOfDate
...
'''
Customerfilter_node1744548110342 = sparkSqlQuery(glueContext, query = SqlQuery8479, mapping = {"customer_trusted":Customerlanding_node1744548099603}, transformation_ctx = "Customerfilter_node1744548110342")
